...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        printf("=====================Site visited so far============================\n");
        for (itDns = dnsIpList.begin(); itDns != dnsIpList.end(); itDns++) {//Print Map
            int i = 0;
            for (; host_match[i].string_to_match != NULL; i++) {
                if (strcasestr((itDns->first).c_str(), host_match[i].string_to_match)) {
                    printf("DNS=%s.com\n", host_match[i].proto_name);
                    break;
                }
            }
            if (host_match[i].string_to_match == NULL) {//Then search for IP
                std::list<string> ipLists = itDns->second;
                std::list<string>::iterator ipIt = ipLists.begin();
                for (; ipIt != ipLists.end(); ipIt++) {
                    uint8_t i1, i2, i3, i4;
                    uint8_t o1, o2, o3, o4;
                    struct in_addr pin, addr;
                    addr.s_addr = inet_addr((*ipIt).c_str());
                    i1 = addr.s_addr & 0xff;
                    i2 = (addr.s_addr >> 8) & 0xff;
                    i3 = (addr.s_addr >> 16) & 0xff;
                    i4 = (addr.s_addr >> 24) & 0xff;
                    for (i = 0; host_protocol_list[i].network; i++) {
                        pin.s_addr = htonl(host_protocol_list[i].network);
                        o1 = pin.s_addr & 0xff;
                        o2 = (pin.s_addr >> 8) & 0xff;
                        o3 = (pin.s_addr >> 16) & 0xff;
                        o4 = (pin.s_addr >> 24) & 0xff;
                        if (i1 == o1 && i2 == o2 && i3 == o3 || (o4 == 0 || o4 == i4)) {
                            printf("DNS Type=%d.com\n", host_protocol_list[i].value);
                            break;
                        }
                    }
                }//Ip list it
                if (ipLists.end() == ipIt) {
                    printf("DNS not found =%s\n", (itDns->first).c_str());
                }
            }
        }
